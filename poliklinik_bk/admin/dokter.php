...
<body>
    <div class="container">
        ...
        <form class="row" method="POST" action="" name="myForm" onsubmit="return validate();">
            ...
            $alamat = '';
            $no_hp = '';
            $id_poli = ''; // Tambahkan kolom id_poli

            if (isset($_GET['id'])) {
                $ambil = mysqli_query($mysqli, "SELECT dokter.*, poli.id AS id_poli FROM dokter 
            ...
            <div class="form-group">
                <label for="inputPoli" class="form-label fw-bold">Poli</label>
                <select class="form-control" name="id_poli" id="inputPoli">
                    <?php
                    $resultPoli = mysqli_query($mysqli, "SELECT * FROM poli");
                    if ($resultPoli) {
                        while ($dataPoli = mysqli_fetch_array($resultPoli)) {
                            $selectedPoli = ($dataPoli['id'] == $id_poli) ? 'selected="selected"' : '';
                    ?>
                            <option value="<?php echo $dataPoli['id'] ?>" <?php echo $selectedPoli ?>><?php echo $dataPoli['nama_poli'] ?></option>
                    <?php
                        }
                    } else {
        ...
        <table class="table table-hover">
            ...
            <tbody>
                <?php
                $result = mysqli_query($mysqli, "SELECT dokter.*, poli.nama_poli FROM dokter 
                                                LEFT JOIN poli ON dokter.id_poli = poli.id");
